...
\end{enumerate}
The following will be captured in specific seperate documents that will refer to this one. 
\begin{enumerate} 
\item 
...
and may also lead to pubcliations.  
\end{enumerate} 
...
\section*{Appendix: Galaxy Clustering Holistic Forecast Definitions}
...
\frac{\partial\ln P_{obs}\left(\bar{z},k,\mu;\theta_{i}\right)}{\partial\ln H(\bar{z})} & =1 \pm ... \comment{check when AP is present} \\
\frac{\partial\ln P_{obs}\left(\bar{z},k,\mu;\theta_{i}\right)}{\partial\ln D_{A}(\bar{z})} & =-2 \pm ... \comment{check}
\end{align}
...
\item Jacobian method (or extended BAO Eisenstein method) \comment{need reference}
\end{itemize}
...
\begin{equation}
V_s = Area (i.e. 15,000 sq deg) * dV_{dz}
\end{equation}
...
\revtext{The Fisher matrix to compare with for this case, is case4.dat.}
