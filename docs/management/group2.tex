...
\cite{WH00} compared the Gaussian contribution to $N$-body simulation results.
...
1 & perfectly known \\
2 & Including dispersion $\sigma_z$, eq.~(\ref{eq:n_binned}) \\
3 & With catastrophic outliers $f_{\rm out}$, calibration $c_{\rm cal}$ and biases $z_{\rm bias}$, eq.~(\ref{eq:n_of_z_calib}) \\
\hline\hline
\end{tabular}
...
\begin{tabular}{ll} \hline\hline
1 & linear, CDM, GR \\
2 & Including non-linear power. \\
3 & Including baryonic corrections \\
4 & Including WDM (neutrinos) \\
5 & non-GR models \\
\hline\hline
\end{tabular}
...
\begin{tabular}{ll} \hline\hline
1  & no IA \\
2a & linear IA model \'a la \cite{2004PhRvD..70f3526H,2007NJPh....9..444B,2011A&A...527A..26J} \\
2b & linear+quadratic alignment models, e.g.~\cite{2013MNRAS.435..194C} \\
\hline\hline
\end{tabular}
...
\begin{tabular}{ll} \hline\hline
1 & Gaussian cosmic variance  (analytical) \\
2 & Including non-Gaussian trispectrum terms \\
3 & Including non-Gaussian super-survey terms \\
\hline\hline
\end{tabular}
...
Lensing projections (see Sect.~\ref{sec:WL_prok})

\begin{tabular}{ll} \hline\hline
1 & Born, Limber, no clustering, shear \\
2 & reduced shear, beyond Limber and Born \\
3 & Including source clustering \\
3 & Relativistic effects, peculiar motions, other exotic higher-order effects \\
\hline\hline
\end{tabular}
...
with massive neutrinos and modified gravity provided some fitting functions are
